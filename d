...
-- Anti-Hook koruma ekle
local protected = {}
for _, funcName in pairs({"getgenv", "getrenv", "getgc", "debug", "hookfunction", "rawset", "rawget", "setfenv"}) do
    protected[funcName] = _G[funcName]
...
-- Global environment tam koruma (Executor uyumlu)
local function setupEnvironmentProtection()
    local success, result = pcall(function()
        local env = getfenv(0)
        -- Mevcut metatable'ı koru
        local existingMt = getmetatable(env)
        local newMt = {}
        
        if existingMt then
            for k, v in pairs(existingMt) do
                newMt[k] = v
            end
        end
        
        -- Yeni __newindex ekle
        newMt.__newindex = function(t, k, v)
            ...
            if type(v) == "table" then
                for tableKey, _ in pairs(v) do
                    if type(tableKey) == "string" and (
                        tableKey:lower():find("bypass") or 
                        tableKey:lower():find("hook") or 
                        tableKey:lower():find("hack")
                    ) then
                        error("Güvenlik ihlali tespit edildi!")
                        return
                    end
            ...
            -- Orijinal __newindex varsa çağır
            if existingMt and existingMt.__newindex then
                return existingMt.__newindex(t, k, v)
            else
                rawset(t, k, v)
            end
        end
        ...
        -- _G için basit koruma
        local _G_mt = {}
        _G_mt.__newindex = newMt.__newindex
        setmetatable(_G, _G_mt)
    end)
    
    if not success then
        warn("Environment koruma hatası:", result)
    end
end

-- Korumayı başlat
setupEnvironmentProtection()
...
-- String şifreleme (obfuscation) fonksiyonu güvenlik eklemesi
local originalObfuscateString = KeyValidator.ObfuscateString
KeyValidator.ObfuscateString = function(self, str)
    -- Debug modu kontrolü
    if debug.getinfo(2) then
        error("Debug modu tespit edildi!")
        return ""
    end
    ...
    if getgenv and type(getgenv) == "function" then
        local env = getgenv()
        if env.bypass or env.hack then
            error("Bypass girişimi tespit edildi!")
            return ""
        end
    end
    
    return originalObfuscateString(self, str)
end

-- Başlatma fonksiyonu güvenlik eklemesi
local originalBaslat = KeyValidator.baslat
function KeyValidator:baslat()
    -- Hook kontrolü
    for name, original in pairs(protected) do
        if _G[name] ~= protected[name] then
            error("Hook tespit edildi: " .. name)
            return nil
        end
    end
    
    -- Global değişken kontrolü
    if _G.bypass or _G.hack then
        error("Bypass girişimi tespit edildi!")
        return nil
    end
    
    return originalBaslat(self)
end
