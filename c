...
mt.__newindex = function(t, k, v)
    ...
    -- Alt tablolarda bypass kontrolü
    if type(v) == "table" then
        local function checkTable(tbl, depth)
            if depth > 5 then return end -- Maksimum derinlik kontrolü
            for key, value in pairs(tbl) do
                if type(key) == "string" and (key:lower():find("bypass") or key:lower():find("hook") or key:lower():find("hack")) then
                    error("Güvenlik ihlali tespit edildi!")
                    return
                end
                if type(value) == "table" then
                    checkTable(value, depth + 1)
                end
            end
        end
        checkTable(v, 1)
